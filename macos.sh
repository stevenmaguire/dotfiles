...
###############################################################################
./bin/menu-bar-clock.sh "EEE d MMM HH:mm:ss"

###############################################################################
# Dock Layout                                                                 #
###############################################################################
echo ""
echo "Would you like to configure the dock?  (y/n)"
read -r response
if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]; then
  ...
  if hash dockutil 2>/dev/null; then
    ...
    if ! dockutil --find "Terminal" > /dev/null; then
      dockutil --add "/Applications/Utilities/Terminal.app"
    fi
    dockutil --move "Terminal" --after 'Finder'


    if ! dockutil --find "Messages" > /dev/null; then
      dockutil --add "/Applications/Messages.app"
    fi
    dockutil --move "Messages" --after 'Terminal'
